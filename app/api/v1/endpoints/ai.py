...
class LearningPlanRequest(BaseModel):
    ...
    time_available: int = Field(..., description="Available time in minutes", gt=0, le=480)
...
class TodoRequest(BaseModel):
    ...
    time_available: int = Field(..., description="Available time in minutes", gt=0, le=480)
    recent_progress: Optional[str] = Field(default=None, description="Recent learning progress")
    weak_areas: Optional[List[str]] = Field(default=None, description="Areas that need improvement")
    daily_goal: Optional[str] = Field(default=None, description="Today's specific goal")


class AnalysisRequest(BaseModel):
    """Request model for progress analysis."""
    period: str = Field(..., description="Analysis period", min_length=1, max_length=100)
    learning_records: str = Field(..., description="Learning records data", min_length=1)
    goals: str = Field(..., description="Learning goals", min_length=1)
    progress_rate: float = Field(..., description="Current progress rate", ge=0.0, le=1.0)


class AdviceRequest(BaseModel):
    """Request model for learning advice."""
    current_issues: str = Field(..., description="Current learning issues", min_length=1)
    learning_status: str = Field(..., description="Current learning status", min_length=1)
...
class GoalRequest(BaseModel):
    ...
    desired_outcome: str = Field(..., description="Desired learning outcome", min_length=1)
    ...
    available_resources: str = Field(..., description="Available resources", min_length=1)
...
async def generate_todo_list(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate TODO list: {str(e)}")
